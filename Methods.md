...
> International Journal of Computer Assisted Radiology and Surgery (2024) 19:1329–1338
>
...
类别对比损失$`\mathcal{L}_{contr}`$利用余弦相似度度量潜在空间中同类 / 异类特征的距离, 其中(N_c=4\)**（四类结构），**\(f_c\)**为记忆库中的投影向量，**\(z_c\)为当前预测向量，通过 “拉近距离同类、推远异类” 增强特征判别性。
...
* 模块将特征图分为 4 组，每组通过两条路径学习注意力：小核路径（1×1×1 卷积）捕捉局部细节，大核路径（3×3×3、5×5×5、7×7×7 卷积）捕捉长距离依赖；
...
冠状动脉具有复杂的树状管状结构，形态曲折，传统方形卷积难以有效捕捉其复杂性并提取有意义的特征；冠状动脉的纤细结构和细小分支导致大量误分割区域，表现为不连续或欠分割，针对管状结构的特定方法（如动态蛇卷积、CoANet）未充分考虑血管的空间分布特征，尤其是血管方向的非严格正交性和大量细小分支的存在
...
**对角线连接约束**，为每个像素考虑四个对角线方向（左上、右上、左下、右下）的邻居点，定义邻居集$`N_{i,j}`$若中心像素与对角邻居均为血管像素，连接性标记为 1，否则为 0, 为每个像素建立4维向量图，通过预测的 4 维连接概率图$`C_{pred}`$**与**真实图$`C_{gt}`$计算交叉熵损失
...
![1752396181225](image/现有方法/1752396181225.png)
...
其中$`L_{neighborhood}`$为邻域损失，通过对骨架及其邻域的平均绝对误差约束骨架细厚度，避免过粗或过细。
...
- 训练目标为条件流匹配（CFM）损失，最小化预测速度与真实速度的L2距离：

  ![1752407135418](image/现有方法/1752407135418.png)
- 生成时通过欧拉积分离散化ODE，使用合成掩码\(M_{syn}\)（无标注误差）作为条件，生成符合类别特征的图像。

**模型训练：**

采用MONAI实现的UNet架构（基于Isensee等人的设计）作为分割模型。训练数据按比例混合：70% Ddrand、20% Dreal、10%Dflow，通过监督学习优化分割损失。

**--结论--**

**实验设置**

- **评估数据集**：4个未见过的域（临床：SMILE-UHURA（MRA）、MSD8（CT）；预临床：OCTA、BvEM（vEM））。
- **任务**：零样本（直接应用）、少样本（用1或3个patch微调）分割。
- **基线模型**：tUbeNet（血管专用）、VISTA3D（CT专用）、SAM-Med3D、MedSAM-2（通用医学分割）。
- **指标**：Dice系数（重叠度）、clDice（拓扑连通性）。

**结果**

![1752407496537](image/现有方法/1752407496537.png)

**图**  定性结果（放大查看效果更好）。我们在 SMILE-UHURA 和 OCTA 数据集上可视化了所有三项任务（零、一和少样本）的预测。我们将 vesselFM 与 VISTA3D、SAM-Med3D 和真值分割掩码进行了比较。请注意，vesselFM 始终如一地预测最先进的结果，即使在零样本设置中也是如此，显示出对看不见领域的特殊泛化。为了获得更好的可见性，我们在 SMILE-UHURA 数据集上展示了 3D 预测的放大视图。

- **零样本任务**：vesselFM显著优于所有基线，例如在SMILE-UHURA上Dice达74.66%（远超SAM-Med3D的2.12%），在BvEM上clDice达62.04%（远超tUbeNet的11.17%）。
- **少样本任务**：微调后性能进一步提升，如OCTA数据集零样本Dice 46.94%，少样本达75.70%。
- **消融实验**：
  - 三大数据源联合训练效果最优：\(D_{real}+D_{drand}+D_{flow}\) 较仅用\(D_{real}\)的Dice提升9.21%，clDice提升11.74%。
  - 流匹配模型（\(D_{flow}\)）优于扩散模型（如Med-DDPM），在SMILE-UHURA上Dice高4.32%，且生成图像更保真。

**--总结--**

vesselFM通过融合真实数据、域随机化合成数据和流匹配生成数据，实现了3D血管分割的通用化。其在零样本、少样本场景下的性能远超现有医学基础模型，尤其在跨模态和复杂血管结构上表现优异。该模型为血管相关疾病（如中风、动脉瘤）的诊断和研究提供了高效工具，开源资源进一步促进了其在临床和科研中的应用。
...
> CVPR 2025
>
> 代码：[https://github.com/WangXueyang-uestc/ARConv](https://github.com/WangXueyang-uestc/ARConv)
>
> 数据集：使用公开数据集 WV3、QB、GF2，可从公开仓库获取，采用 Wald 协议处理以构建训练 / 测试集。

**动机：**

针对遥感图像，传统卷积的采样位置**局限于固定正方形**窗口，无法自适应不同物体的形状和尺度；卷积核的**采样点数量预设不变**，难以适应遥感图像中物体的显著尺度差异（如小汽车与大型建筑），导致特征提取效率低下。

**创新点：**

1. 提出 **自适应矩形卷积（ARConv）** ：可自适应学习卷积核的高度和宽度，并根据学习到的尺度动态调整采样点数量，有效捕捉遥感图像中不同尺度物体的特征。
2. 构建 **ARNet 网络** ：基于 U-Net 架构，用 ARConv 替代标准卷积层，增强对多尺度特征的提取能力，优化 pansharpening 效果。
3. 通过热图可视化验证了学习到的卷积核尺寸与实际物体大小的相关性，证实了 ARConv 对物体尺度的自适应能力。

**总结：**

关键是自适应的矩形卷积，和蛇形卷积一样是可变形的卷积，可能可以作为一种替代。

---

### Centerline Boundary Dice Loss for Vascular Segmentation

> MICCAI 2024, LNCS 15008, pp. 46–56, 2024.
>
> 代码：[https://github.com/PengchengShi1220/cbDice](https://github.com/PengchengShi1220/cbDice)
>
> 数据集：使用公开数据集 DRIVE、Parse 2022、TopCoW 2023
